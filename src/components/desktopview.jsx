...
export function Desktop(){
    ...
    const a=useContext(context)
     ...
     const[self,setself]=useState(a.self)    
   ...
   const socket =io('http://192.168.29.37:4000/')
     const callf=async()=>
     { await axios.get(`http://192.168.29.37:4000/getcoversations/${a.self._id}`).then
      (res => {
...
return( <div>
    ...
    <div id="navsearch" style={{backgroundColor:`${background_search}`}} onClick={()=>{set_search_or_chat(false);setbackground_search('white');setbackground_chat('gray')}}>search</div>
       ...
       <>
        <Navbar open={set_open_setting} src={`http://192.168.29.37:4000/`+a.self.profile_picture||'https://butwhytho.net/wp-content/uploads/2023/09/Gojo-Jujutsu-Kaisen-But-Why-Tho-2.jpg'}></Navbar>
          <div id="home">
            <div id='pcontainer'>
           {open_setting?<Settings close={set_open_setting} src={`http://192.168.29.37:4000/`+a.self.profile_picture||'https://butwhytho.net/wp-content/uploads/2023/09/Gojo-Jujutsu-Kaisen-But-Why-Tho-2.jpg'}></Settings> :<><div id='profile' onClick={()=>{set_open_setting(true)}}>
                <div>
                   
                <img id="dp" src={`http://192.168.29.37:4000/`+a.self.profile_picture||'https://butwhytho.net/wp-content/uploads/2023/09/Gojo-Jujutsu-Kaisen-But-Why-Tho-2.jpg'}></img>
                <h3>{a.self.Name}</h3>
                </div>
            </div>
            <></>
